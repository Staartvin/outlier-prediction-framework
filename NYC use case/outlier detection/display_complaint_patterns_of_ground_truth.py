...
def convert_dataframe_to_displayable_dataframe(
    ...
    zone_set = dataframe["zone set"].iat[0]
...
def filter_complaint_data(zone_set="North Manhattan") -> DataFrame:
    ...
    for week_number in range(1, last_week_of_a_year(2018) + 1):
        dates_to_match.extend(dates_of_particular_week(2018, week_number))
    ...
    mask = complaint_data["Closed Date"].dt.date.isin(dates_to_match)
